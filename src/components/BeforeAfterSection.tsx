...
export default function BeforeAfterSection() {
  ...
  const activeResult = cases[activeCase];
  ...
  const handleSliderMove = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    const percentage = Math.max(0, Math.min(100, ((clientX - container.left) / container.width) * 100));
          ...
          <div className="bg-white rounded-2xl shadow-xl p-4 sm:p-6 border border-gray-100">
            ...
            >
              <AnimatePresence mode="wait" custom={direction}>
                <motion.div key={activeCase} custom={direction}
                  initial={{ opacity: 0, x: direction * 60 }} animate={{ opacity: 1, x: 0 }}
                  exit={{ opacity: 0, x: direction * -60 }} transition={{ duration: 0.35, ease: 'easeInOut' }}
                  className="absolute inset-0">
                  <img src={activeResult.afterImg} alt={`${activeResult.title} - After`} referrerPolicy="no-referrer"
                  ...
                  <div className="absolute right-4 bottom-4 bg-[#25D366]/90 backdrop-blur-sm text-white text-xs font-bold px-3 py-1.5 rounded-full flex items-center space-x-1.5 shadow-md">
                    <Check className="w-3.5 h-3.5" /><span>{t.beforeAfter.labelAfter}</span>
                  </div>
                  <div className="absolute inset-y-0 left-0 overflow-hidden pointer-events-none" style={{ width: `${sliderPosition}%` }}>
                    <img src={activeResult.beforeImg} alt={`${activeResult.title} - Before`} referrerPolicy="no-referrer"
                    ...
                    <div className="absolute left-4 bottom-4 bg-red-600/90 backdrop-blur-sm text-white text-xs font-bold px-3 py-1.5 rounded-full flex items-center space-x-1.5 shadow-md">
                      <Eye className="w-3.5 h-3.5" /><span>{t.beforeAfter.labelBefore}</span>
                    </div>
                  </div>
            ...
            <div className="mt-6 text-center space-y-2">
              <h3 className="text-xl font-bold text-neutral-charcoal">{activeResult.title}</h3>
              ...
              <div className="flex items-center gap-2">
                {cases.map((_, idx) => (
                  <button key={idx}
                    ...
                    className={`rounded-full transition-all duration-300 cursor-pointer ${idx === activeCase ? 'w-6 h-2.5 bg-primary-blue' : 'w-2.5 h-2.5 bg-gray-300 hover:bg-gray-400'}`}
